...
function conDown(){
	if($('input.con_invcode').val().length!=0){
		...
		$.ajax({
			type:"get",
			url:"/ProductionLineSignage/rest/upload/dispatchFiles?productionline=C4&invcode="+$('input.con_invcode').val(),
			dataType:"json",
			success:function(e){
...
function opeDown(){
	if($('input.ope_invcode').val().length!=0){
		$.ajax({
			type:"get",
			url:"http://192.168.3.21:8089/ProductionLineSignage/rest/upload/reqDispatchFileDetails?invcode="+$('input.ope_invcode').val(),
			dataType:"json",
			success:function(e){
...
$('#thumbnail>div').on('click','dl',function(){
	...
	var dlUrl="http://192.168.3.21:8089/ProductionLineSignage/rest/upload/reqpdffile?path="+$(this).children('dt').html();
	$.ajax({
		type:"get",
		url:dlUrl,
		success:function(){
...
$(function(){
	$('input.ope_query').click(function(){
		...
		$.ajax({
			type:"post",
			url:"http://192.168.3.21:8089/ProductionLineSignage/rest/upload/setConfigure",
...
//      $("#fileForm").ajaxSubmit({
//          type: "post",
//          url: "rest/upload/uploadProductCraftResource",
//          success: function (backdata) {
//              $("#popup p").html(backdata.return_msg);
//          },
//          error: function () {
//              $("#popup p").html("文件上传失败!");
//          }
